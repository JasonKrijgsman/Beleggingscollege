...
> **`AGENTS.md` en `CLAUDE.md` horen dezelfde productwaarheid te bevatten — werk ze samen bij.** Tool-specifieke afspraken mogen verschillen, feiten over de code niet. Dat is aantoonbaar al een keer misgegaan: tot 3 aug 2026 beschreef dit bestand nog het oude `purchases`-model en beweerde het dat voortgang alleen in localStorage leefde — precies het geldpad en het voortgangspad. Wijk je hier iets af, controleer dan of het andere bestand mee moet.
...
- **`beleggingscollege.nl`** — **DNS verhuisd op 3 aug 2026.** DNSSEC is eraf (geen DS-record meer bij de registry) en de delegatie staat op `joan.ns.cloudflare.com` / `rene.ns.cloudflare.com`. Wat nog wél bij Strato hangt is de **registrar**: de Providerwissel-wizard faalde twee keer, en zonder verhuiscode geen transfer. Geen haast — het domein loopt tot 20-08-2027. Zie `docs/domain-migration-plan.md`.
- **De `.nl` serveert nog steeds de oude WordPress-site**, inclusief drie verzonnen testimonials die we juist uit de nieuwe site hebben gehaald — en dat is het adres in onze eigen footer en op elk certificaat. De A-records in de Cloudflare-zone wijzen nog naar `81.169.145.93` bij Strato. **Dat omzetten wacht níét op de verhuiscode**; het stappenplan met de valkuilen staat in `docs/nl-als-hoofddomein.md` en dit is de eerstvolgende concrete domeinactie.
- **Zodra de .nl de site serveert**: `NEXT_PUBLIC_SITE_URL` in Vercel op `https://beleggingscollege.nl` zetten en een permanente redirect `.com` → `.nl` toevoegen. Verder hoeft er niets in de code.
...
- **De post draait sinds 3 aug 2026 bij Migadu** (Actief sinds 15:51 UTC). De MX wijst naar `aspmx1`/`aspmx2.migadu.com`, `beheer@beleggingscollege.nl` bestaat er en ontvangt; de oude Strato-postbussen zijn met de naamserverwissel vervallen en die inhoud kon gemist worden. **Er blijft één stap over vóór er post uitgaat:** `MAIL_SMTP_GEBRUIKER` en `MAIL_SMTP_WACHTWOORD` in Vercel. Zolang die leeg zijn logt de app een waarschuwing en gaat er niets de deur uit — de aankoop werkt gewoon door.
- **Valkuil die stil misging (opgelost 3 aug 2026):** het domein publiceerde `p=reject` zónder SPF, waardoor élke mail van een nieuwe verzender geweigerd zou worden — niet in spam, geweigerd. SPF, alle drie de DKIM-CNAME's en DMARC staan er nu, en DMARC staat **bewust tijdelijk op `p=quarantine`**. De volgorde die dat gat dichthoudt: eerst één echte bestelling testen, dán DMARC terug naar `p=reject`. Zet hem niet terug voordat verzenden bewezen werkt.
