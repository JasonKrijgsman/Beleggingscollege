...
- **Werkwijze (sinds 3 aug 2026): werk op een branch en merge via een PR met groene CI — direct naar `main` pushen wordt geweigerd.** Main is beschermd (vereiste check "CI", ook voor admins) omdat elke merge naar productie deployt. Lokaal reproduceer je de hele poort met `npm ci && npm run controle`; zie `docs/ci.md`.
