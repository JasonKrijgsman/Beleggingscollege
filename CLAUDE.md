...
- **De winkel staat sinds 5 aug 2026 DICHT: er is géén `MOLLIE_API_KEY` in Production.** De checkout geeft 503 "Betalen is nog niet ingeschakeld" — bewust, omdat de site sindsdien bij Google wordt aangeboden en de test-key anders een open deur was (iedereen kon via Mollie's testmodus gratis cursussen ontgrendelen). Preview hééft de test-key nog: betaalketen testen doe je op een PR-preview-URL. Open doen = key (live óf test) in Vercel Production + redeploy — maar Jason wil eerst zelf alles nalopen; niet openen zonder zijn expliciete keuze. Stand: `docs/openstaand.md` §1.
