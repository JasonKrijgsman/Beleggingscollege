...
| **Webhook verifieert bedrag én valuta** | ⚠️ **Ingetrokken na verificatie (17).** Dit stond hier eerst als "de marktleider doet dit niet". Onjuist voor 5.1.8: de PayPal-keten controleert sinds 4.20.1 valuta én bedrag tegen de vastgelegde prijs en trekt bij verschil toegang in. Bij Stripe/Razorpay ontbreekt de controle omdat het bedrag daar server-side uit de catalogus komt en nooit langs de klant reist — geen gat, maar ontwerp. **Wij zijn gelijkwaardig, niet strenger.** Onze regel blijft goed; de zelffelicitatie ging eruit. | 10, **17** |
| **`server-only` als bouwtijdgrens** | En dit is juist sterker dan we dachten. Hun contentbescherming hangt niet alleen aan een `the_content`-filter, maar die filter is via `learndash_template_preprocess_filter` **uit te schakelen** — en hun eigen Elementor-add-on doet dat. Een integratie kan bij hen dus de afscherming van betaalde lesinhoud opheffen. Bij ons kán een import geen server-grens passeren: de build faalt. | 10, **17** |
| **Gedeeltelijke terugbetaling laat toegang staan** | **Half weerlegd — lees dit vóór je het verdedigt.** WooCommerce trekt bij een gedeeltelijke terugbetaling precies de terugbetaalde regelitems in; ThriveCart maakt er een instelling van met intrekken als standaard. Onze keuze klopt zolang één betaling altijd één cursus is, maar die voorwaarde staat nergens vastgelegd en vervalt bij de eerste bundel. Wat wél zuiver bevestigd wordt: dat een gedeeltelijk terugbetaalde order `completed` blijft en je dus een aparte refund-hook nodig hebt — hun code draagt daar hetzelfde commentaar over als wij. | 13 |
...
| **Native gamification** (XP, 8 levels, streaks, 10 badges) | Core-LearnDash heeft *niets* hiervan; alles komt uit add-ons, en echte streaks bestaan zelfs daar nauwelijks. De broncode maakt het verschil nog scherper: hun "streak" is één usermeta-teller die bij inloggen +1 doet — een **login**-reeks, geen leerreeks, en hij beloont het openen van een tab. De onze telt dagen waarop echt een les is afgerond. | 05 §3, **14** |
...
**En dankzij hoofdstuk 14 weten we nu hoe ze het écht gebouwd hebben — inclusief wat we níét moeten overnemen.** Hun wachtrij is een tabel met een verzendtijdstip, en dat idee is goed; de uitvoering is een verzameling waarschuwingen:

- **Wat we kopiëren:** een rij per geplande mail, en in die rij **id's in plaats van uitgeschreven tekst** (zij lezen titel en inhoud pas bij verzending opnieuw op, zodat een tekstcorrectie ook de wachtende mails haalt). En hun keuze om voor de inactiviteitsmail niet op "laatste login" te vertrouwen maar op echte activiteit — een Remember-me-sessie maakt "laatste login" waardeloos. Wij hebben die activiteit al in `user_stats`.
- **Wat we beter doen:** hun ontdubbeling is een usermeta-vlag plus een `REGEXP` over een geserialiseerde kolom, zónder unique constraint of slot — dus twee gelijktijdige cron-runs kunnen dezelfde mail tweemaal sturen. Wij hebben hier al het juiste patroon voor: de atomaire claim op `confirmationClaimedAt` in de orderbevestiging. Dat patroon hergebruiken, niet hun vlag.
- **Wat ronduit een bug is:** ze plannen **één** cron-event voor alleen de eerstvolgende rij, dus tien wachtende mails kosten tien cron-runs; een mislukte verzending wordt nooit opnieuw geprobeerd (de rij verdwijnt hoe dan ook); en de "stuur deze herinnering maar één keer"-rem leest een meta-sleutel die nergens geschreven wordt. Zelfs de haaknaam bevat een typefout die ze niet meer kunnen repareren zonder de add-on-markt te breken — een klein monument voor het kostenplaatje van publieke interfaces.

Netto: **de tabelvorm overnemen, de verwerking niet.** Eén query die alle openstaande rijen pakt, een atomaire claim per rij, en een mislukking die terug de wachtrij in gaat.
...
Uit de commerce-add-ons (13): **alle vier de serieuze integraties houden een referentieteller bij** — per gebruiker per cursus een lijst van order-id's die dat recht onderbouwen. Toegang wordt pas ingetrokken als de láátste bron wegvalt. Wij hebben dat niet: `entitlements` heeft één rij per gebruiker per cursus, zonder te weten waaróm.

Zolang we alleen losse cursussen verkopen is dat prima. Zodra College+ bestaat, niet meer: wie Waardebeleggen los kocht én daarna een abonnement neemt, verliest bij opzeggen de cursus die hij **apart betaald heeft**. Stil, zonder foutmelding, en pas zichtbaar als de klant mailt. Het is precies het soort fout dat wij normaal in een invariant vangen.
